...
import { UserAdminService, PermissionDto } from '../../../users/data-access/user-admin.service';
...
export class PermissionsListComponent implements OnInit {
  ...
  confirmOpen = false;
  deleting = false;
  itemToDelete: PermissionDto | null = null;
  ...
  fetch() {
    ...
    this.admin.listPermissions({ search: this.query || undefined, isActive: isActiveParam, page: 1, pageSize: 1000 }).subscribe({
      next: (res: any) => {
      ...
      error: () => { this.error = 'İzinler yüklenemedi'; this.loading = false; }
    });
  }
  ...
  getPagedItems(): PermissionDto[] { const start = (this.page - 1) * this.pageSize; const end = start + this.pageSize; return this.items.slice(start, end); }

  openDeleteConfirm(item: PermissionDto) { this.itemToDelete = item; this.confirmOpen = true; }
  cancelDelete() { this.confirmOpen = false; this.itemToDelete = null; this.deleting = false; }
  confirmDelete() {
    if (!this.itemToDelete) return;
    this.deleting = true;
    this.admin.deletePermission(this.itemToDelete.id).subscribe({
      next: () => { this.items = this.items.filter(i => i.id !== this.itemToDelete!.id); this.total = this.items.length; this.toastr.success('İzin silindi'); this.cancelDelete(); },
      error: () => { this.toastr.error('İzin silinemedi'); this.deleting = false; }
    });
  }
}