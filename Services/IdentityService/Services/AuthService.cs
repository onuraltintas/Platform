    ...
    public async Task<ApiResponse<AuthResponse>> LoginAsync(LoginRequest request, string? ipAddress = null)
    {
        ...
        try
        {
            ...
            var user = await _context.Users
                ...
                .FirstOrDefaultAsync(u => isEmail ? u.Email == request.EmailOrUsername : u.UserName == request.EmailOrUsername);
      ...
      <tr>
        <td align='center' style='padding:32px 12px;'>
          <table role='presentation' cellpadding='0' cellspacing='0' width='100%' style='max-width:560px;background:#ffffff;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.06);overflow:hidden;'>
            ...
            try
            {
                _logger.LogInformation("About to call email service for {Email}", user.Email);
                ...
                var emailResult = await _emailService.SendEmailAsync(user.Email, emailSubject, emailBody, isHtml: true);
                
                if (emailResult.IsSuccess)
                {
                    _logger.LogUserAction(user.Id, "EmailConfirmationSent", new { Email = request.Email });
                    _logger.LogInformation("Successfully sent confirmation email to {Email}", user.Email);
                }
                else
                {
                    _logger.LogWarning("Failed to send confirmation email to {Email}: {ErrorMessage}", user.Email, emailResult.ErrorMessage);
                }
            }
...
<body>
    ...
    public async Task<ApiResponse<object>> ChangeUserNameAsync(string userId, ChangeUserNameRequest request)
    {
        try
        {
            ...
            var exists = await _context.Users.AnyAsync(u => u.UserName == request.NewUserName && u.Id != userId);
            if (exists)
            {
      ...
      <tr>
        <td align='center' style='padding:32px 12px;'>
          <table role='presentation' cellpadding='0' cellspacing='0' width='100%' style='max-width:560px;background:#ffffff;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,0.06);overflow:hidden;'>
            ...
            try
            {
                _logger.LogInformation("Attempting to resend confirmation email to {Email}", user.Email);
                var emailResult = await _emailService.SendEmailAsync(user.Email, emailSubject, emailBody, isHtml: true);
                
                if (emailResult.IsSuccess)
                {
                    _logger.LogUserAction(user.Id, "ResendEmailConfirmationSent", new { Email = email });
                    _logger.LogInformation("Successfully resent confirmation email to {Email}", user.Email);
                }
                else
                {
                    _logger.LogWarning("Failed to resend confirmation email to {Email}: {ErrorMessage}", user.Email, emailResult.ErrorMessage);
                }
            }
            catch (Exception emailEx)
            {
                _logger.LogError(emailEx, "Exception occurred while resending confirmation email to {Email}", user.Email);
            }
